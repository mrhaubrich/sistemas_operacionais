...
void *worker_func(void *arg) {
    ...
    //     size_t header_len = 0;
    //     int dq = thread_safe_queue_dequeue(queue, &slice, &slice_len,
    //     &header,
    //                                        &header_len);
    //     if (dq != 0) {
    //         // Queue is empty
    ...
    //     int received =
    //         buffer ? receive_processed_csv(&uds_info, buffer, buffer_size) :
    //         -1;
    //     if (received > 0) {
    //         int lines = 0;
...
int main(int argc, char *argv[]) {
    ...
    CSVFile csvFile = csvfile_map(filepath);
    if (csvFile.header.start_offset == csvFile.header.end_offset) {
        fprintf(stderr, "Falha ao mapear o arquivo\n");
        return EXIT_FAILURE;
    }

    // Imprimir informações do arquivo
    csvfile_print_info(&csvFile);
    ...
    if (!queue) {
        ...
        csvfile_unmap(&csvFile);
        return EXIT_FAILURE;
    }

    int calculated_size =
        csvFile.line_count / n_cpus + (csvFile.line_count % n_cpus != 0);
    int chunk_size = (calculated_size < 100000) ? calculated_size : 100000;

    printf("Tamanho do chunk: %d\n", chunk_size);

    size_t num_chunks = partition_csv(&csvFile, chunk_size, queue);
    ...
    // printf("\nPrimeiras 10 linhas retornadas por todos os CSVs
    // processados:\n"); for (size_t i = 0; i < n_cpus; ++i) {
    ...
    //             pos++; size_t len = pos - start; if (len > 0 ||
    ...
    // printf("\nTotal de linhas retornadas por todos os CSVs processados:
    // %d\n",
    //        total_lines);
